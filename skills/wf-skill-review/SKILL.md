...
1. **Mark task done in sprint file.** Append ` [DONE]` to the task heading in the sprint file (`paths.sprint` from config).
...
8. **Report to human.** Inform the human the branch is pushed and ready for PR/merge. Ask what to do next: plan the next task, or run analysis for the next sprint.
